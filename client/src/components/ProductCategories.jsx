...
const CategoryLink = ({ icon, title }) => (
  <Link to={`/category/${title.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`} className="flex items-center rounded-lg border border-gray-200 bg-white px-4 py-2 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
  ...
  const categories = [
    {
      ...
      icon: (
        <svg className="h-4 w-4 shrink-0 text-gray-900 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
    ...
    <section className="bg-gray-50 py-8 antialiased dark:bg-gray-900 md:py-16">
      <div className="mx-auto max-w-screen-xl px-4 2xl:px-0">
        <div className="mb-4 flex items-center justify-between gap-4 md:mb-8">
          ...
          <Link to="/categories" className="flex items-center text-base font-medium text-primary-700 hover:underline dark:text-primary-500">
            See more categories
        ...
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {categories.map((category, index) => (
            <CategoryLink key={index} icon={category.icon} title={category.title} />
          ))}
        </div>
      </div>
    </section>
